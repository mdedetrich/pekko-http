...
public class WebSocketClientExampleTest {
  ...
  public void testSingleWebSocketRequest() {
    ...
    final Source<Message, NotUsed> helloSource = Source.single(TextMessage.create("hello world"));
    ...
    final Pair<CompletionStage<WebSocketUpgradeResponse>, CompletionStage<Done>> pair =
        http.singleWebSocketRequest(
            WebSocketRequest.create("ws://echo.websocket.org"), flow, materializer);
    ...
    final CompletionStage<Done> connected =
        pair.first()
            .thenApply(
                upgrade -> {
                  // just like a regular http request we can access response status which is
                  // available via upgrade.response.status
  ...
  public void halfClosedWebSocketClosingExample() {
    ...
    final Flow<Message, Message, NotUsed> flow =
        Flow.fromSinkAndSource(Sink.foreach(System.out::println), Source.empty());

    http.singleWebSocketRequest(
        WebSocketRequest.create("ws://example.com:8080/some/path"), flow, materializer);
  ...
  public void halfClosedWebSocketWorkingExample() {
    ...
    final Flow<Message, Message, CompletableFuture<Optional<Message>>> flow =
        Flow.fromSinkAndSourceMat(Sink.foreach(System.out::println), Source.maybe(), Keep.right());

    final Pair<CompletionStage<WebSocketUpgradeResponse>, CompletableFuture<Optional<Message>>>
        pair =
            http.singleWebSocketRequest(
                WebSocketRequest.create("ws://example.com:8080/some/path"), flow, materializer);
  ...
  public void halfClosedWebSocketFiniteWorkingExample() {
    ...
    final Flow<Message, Message, CompletableFuture<Optional<Message>>> flow =
        Flow.fromSinkAndSourceMat(Sink.foreach(System.out::println), source, Keep.right());
  ...
  public void testWebSocketClientFlow() {
    ...
    // send this as a message over the WebSocket
    Source<Message, NotUsed> helloSource = Source.single(TextMessage.create("hello world"));
    ...
    CompletionStage<Done> connected =
        upgradeCompletion.thenApply(
            upgrade -> {
              // just like a regular http request we can access response status which is available
              // via upgrade.response.status
              // status code 101 (Switching Protocols) indicates that server support WebSockets
              if (upgrade.response().status().equals(StatusCodes.SWITCHING_PROTOCOLS)) {
                return Done.getInstance();
              } else {
  ...
  public void testSingleWebSocketRequestWithHttpsProxyExample() {
    ...
    final Flow<Message, Message, NotUsed> flow =
        Flow.fromSinkAndSource(
            Sink.foreach(System.out::println), Source.single(TextMessage.create("hello world")));

    ClientTransport proxy =
        ClientTransport.httpsProxy(InetSocketAddress.createUnresolved("192.168.2.5", 8080));
  ...
  public void testSingleWebSocketRequestWithHttpsProxyExampleWithAuth() {
    ...
    InetSocketAddress proxyAddress = InetSocketAddress.createUnresolved("192.168.2.5", 8080);
