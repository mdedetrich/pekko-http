...
public class JavaTestServer {
  ...
  public Route createRoute() {
    ...
    final Function<Optional<ProvidedCredentials>, Optional<String>> handleAuth =
        (maybeCreds) -> {
          if (maybeCreds.isPresent()
              && maybeCreds.get().verify("pa$$word")) // some secure hash + check
    ...
    final Unmarshaller<ByteString, JavaTweet> JavaTweets =
        Jackson.byteStringUnmarshaller(JavaTweet.class);
    final Route tweets =
        path(
            "tweets",
            () ->
                        ...
                        parameter(
                            ...
                            n -> {
                              ...
                              return completeOKWithSource(
                                  tws, Jackson.marshaller(), EntityStreamingSupport.json());
                            }))
                    .orElse(
                        post(
                            () ->
                                extractMaterializer(
                                    mat ->
                                        entityAsSourceOf(
                                            ...
                                            null,
                                            sourceOfTweets -> {
                                              final CompletionStage<Integer> tweetsCount =
                                                  sourceOfTweets.runFold(
                                                      0, (acc, tweet) -> acc + 1, mat);
                                              return onComplete(
                                                  tweetsCount,
                                                  c -> complete("Total number of tweets: " + c));
                                            })))));

    final Route inner = path("inner", () -> getFromResourceDirectory("someDir"));
  ...
  private void run() throws InterruptedException {
    ...
    final CompletionStage<ServerBinding> binding =
        Http.get(system).newServerAt("127.0.0.1", 8080).bind(createRoute());
